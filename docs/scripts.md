...
| `fm-ensure-agents-md.sh` | Ensure the project memory convention - the default real `AGENTS.md` plus `CLAUDE.md` `@AGENTS.md` pointer, or a repository's own correct `AGENTS.md` -> `CLAUDE.md` alias when present - and the canonical self-governance section |
